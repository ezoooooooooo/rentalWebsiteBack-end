...
const addToCart = async (req, res) => {
  try {
    ...
    if (existingItem) {
      // Instead of updating, return a message that it's already in the cart
      ...
      await cart.save();
      await cart.populate('items.listing', 'name rentalRate images');  // Populate listing details
...
const getCart = async (req, res) => {
 try {
  const userId = req.user?.userId;
   ...
   const cart = await Cart.findOne({ user: userId }).populate('items.listing', 'name rentalRate images');

   if (!cart || cart.items.length === 0) {
     return res.status(200).json({ success: true, message: "Cart is empty", cart: { user: userId, items: [] }, totalPrice: 0 });
   }

   // Calculate total price
   const totalPrice = cart.items.reduce((sum, item) => sum + (item.listing.rentalRate * item.rentalDays), 0);

   res.status(200).json({ success: true, cart, totalPrice: totalPrice.toFixed(2) });
 } catch (error) {
...
console.log("Item ID to remove:", itemId);
    ...
    cart.items.forEach(item => console.log("Item ID in cart:", item._id, "Listing ID:", item.listing));
